...
- **Revenue Import Strategy**: 
  - **CSV Import (Primary for Historical Data)**: The Mindbody `/sale/sales` API endpoint ignores date range parameters and only returns recent sales (~10-20 records), making it unreliable for historical data imports. CSV export from Mindbody Business Intelligence reports is the authoritative source for historical revenue data. Features flexible column mapping (Date/Description/Amount/Type), client matching by Mindbody ID/email/name, duplicate prevention, bulk processing with progress tracking, and validation. Successfully handles large datasets (22,000+ records).
  - **API Import (For Recent Sales Only)**: Use Mindbody API for importing last 24-48 hours of sales data only. Extracts line-item details from `PurchasedItems` with Service/Product categorization via `IsService` field. Automatically skips zero-amount transactions (ClassPass/GymPass redemptions, complimentary entries). Best used with scheduled daily imports or webhooks for ongoing synchronization.
